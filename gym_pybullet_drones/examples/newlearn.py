...
from gym_pybullet_drones.envs.MultiSwarmAviary import MultiSwarmAviary
...
def run(output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True):
    ...
    train_env = make_vec_env(MultiSwarmAviary,
    ...
    eval_callback = EvalCallback(eval_env,
                                 ...
                                 deterministic=True,
                                 render=False)
    model.learn(total_timesteps=3*int(1e4) if local else int(1e2), # shorter training in GitHub Actions pytest
    ...
    test_env = MultiSwarmAviary(gui=gui,
                                    num_drones=DEFAULT_AGENTS,
                                    obs=DEFAULT_OBS,
                                    act=DEFAULT_ACT,
                                    record=record_video)
    test_env_nogui = MultiSwarmAviary(num_drones=DEFAULT_AGENTS, obs=DEFAULT_OBS, act=DEFAULT_ACT)
