...
class HW2Control():
    ...
    def compute_control(self,
        ...
        print("self.last_rpy: ", self.last_rpy)
        ...
        propellers_1_rpm = np.sqrt(np.abs(propellers_rpm[1, 0]))
        propellers_3_rpm = np.sqrt(np.abs(propellers_rpm[2, 0]))

        #### For motion in the Y-Z plane, assign the same turn rates to prop. 0 and 2
        propellers_0_and_2_rpm = np.sqrt(np.abs(propellers_rpm[0, 0]))
